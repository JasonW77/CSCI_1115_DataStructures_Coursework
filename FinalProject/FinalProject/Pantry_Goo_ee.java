...
public class Pantry_Goo_ee extends Application {
	...
	public void start(Stage primaryStage){
		...
		/*java.io.File file = new java.io.File("pantry.txt");
		

		try (
			//read from file "pantry.dat"
			ObjectInputStream ois = new ObjectInputStream(new FileInputStream(file));
			){
				map.put(ois.readObject());
		...
		*/
		...
		btFinish.setOnAction(e -> {
			...
			/*
			try (
				ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("pantry.txt", true));
				){
				oos.writeObject(map);
